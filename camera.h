...
public:
	...
	void Update(const glm::vec3& position, const glm::vec3& center)
	{
		this->position = position;
	...
	void MoveUpward(float distance) //useful function when moving the plane
	{
	...
	void RotateFirstPerson_OX(float angle)
	{
		...
		up = glm::vec3(glm::normalize(glm::rotate(glm::mat4(1.f), angle, right) * glm::vec4(up, 0)));
	...
	void RotateFirstPerson_OY(float angle)
	{
		...
		glm::mat4 rot = glm::rotate(glm::mat4(1.f), angle, glm::vec3(0, 1, 0));
		forward = glm::vec3(glm::normalize(rot * glm::vec4(forward, 0)));
		right = glm::vec3(glm::normalize(rot * glm::vec4(right, 0)));
